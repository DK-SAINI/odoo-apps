...
class MCPController(http.Controller):
    ...
    def _run_with_guard(self, callback, auth_required=True):
        ...
        try:
            ...
            if auth_required:
                ...
                if not (user.has_group('mcp_server.group_mcp_user') or user.has_group('mcp_server.group_mcp_admin')):
                    return _error_response(
                        'ACCESS_DENIED',
    ...
    def _get_max_records(self):
        ...
        ICP = request.env['ir.config_parameter'].sudo()
        return int(ICP.get_param('mcp_server.max_records_per_request', '1000'))
    ...
    def auth_validate(self, **kwargs):
        ...
        def _handler(uid, user):
            ...
            data = {
                ...
                'is_mcp_admin': user.has_group('mcp_server.group_mcp_admin'),
                'is_mcp_user': user.has_group('mcp_server.group_mcp_user'),
            }
            duration = (time.time() - start) * 1000
    ...
    def search_records(self, model, **kwargs):
        ...
        def _handler(uid, user):
            ...
            # Check cache
            ICP = request.env['ir.config_parameter'].sudo()
            cache_enabled = ICP.get_param('mcp_server.cache_enabled', 'False').lower() in ('true', '1')
            cache_ttl = 0
            if cache_enabled and access and access.cache_ttl > 0:
                cache_ttl = access.cache_ttl
            elif cache_enabled:
                cache_ttl = int(ICP.get_param('mcp_server.default_cache_ttl', '300'))
